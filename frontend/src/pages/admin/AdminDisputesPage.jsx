...
const AdminDisputesPage = () => {
  ...
  useEffect(() => {
    adminService.getDisputes().then(({ data }) => setDisputes(data.data)).finally(() => setLoading(false));
  }, []);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900">Dispute Mediation</h1>
        <p className="text-sm text-gray-500 mt-0.5">{disputes.length} disputes platform-wide</p>
      </div>
